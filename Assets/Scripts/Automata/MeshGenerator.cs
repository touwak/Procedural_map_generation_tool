...
public class MeshGenerator : MonoBehaviour {
  ...
  //params it to no especific number of components
  ...
  //        Gizmos.DrawCube(squareGrid.grid[x, y].topLeft.position, Vector3.one * .4f);
                                     
  //        Gizmos.color = (squareGrid.grid[x, y].topRight.active) ? Color.black : Color.white;
  //        Gizmos.DrawCube(squareGrid.grid[x, y].topRight.position, Vector3.one * .4f);
                                     
  //        Gizmos.color = (squareGrid.grid[x, y].bottomRight.active) ? Color.black : Color.white;
  //        Gizmos.DrawCube(squareGrid.grid[x, y].bottomRight.position, Vector3.one * .4f);
                                   
  //        Gizmos.color = (squareGrid.grid[x, y].bottomLeft.active) ? Color.black : Color.white;
  //        Gizmos.DrawCube(squareGrid.grid[x, y].bottomLeft.position, Vector3.one * .4f);
                                     
  //        Gizmos.color = Color.grey; 
  //        Gizmos.DrawCube(squareGrid.grid[x, y].centreTop.position, Vector3.one * .15f);
  //        Gizmos.DrawCube(squareGrid.grid[x, y].centreRight.position, Vector3.one * .15f);
  //        Gizmos.DrawCube(squareGrid.grid[x, y].centreBottom.position, Vector3.one * .15f);
  //        Gizmos.DrawCube(squareGrid.grid[x, y].centreLeft.position, Vector3.one * .15f);
    ...
    public SquareGrid(int[,] map, float squareSize) {
      ...
      for (int x = 0; x < nodeCountX - 1; x++) {
        for (int y = 0; y < nodeCountY - 1; y++) {
          ...
          grid[x, y] = new Square(controlNodes[x, y + 1], controlNodes[x + 1, y + 1],
  ...
  public class Square {
    ...
    public Square(ControlNode _topLeft, ControlNode _topRight, 
      ...
      centreTop = topLeft.rigth;
      centreRight = bottomRight.above;
      centreBottom = bottomLeft.rigth;
  ...
  public class ControlNode : Node {
    ...
    public Node above, rigth;

    public ControlNode(Vector3 _pos, bool _active, float squareSize) : base(_pos) {
      active = _active;
      above = new Node(position + Vector3.forward * squareSize / 2f);
      rigth = new Node(position + Vector3.right * squareSize / 2f);
    }
