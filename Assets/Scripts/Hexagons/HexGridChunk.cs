...
public class HexGridChunk : MonoBehaviour {
  ...
  void TriangulateWithoutRiver(HexDirection direction, HexCell cell, 
  ...
  void TriangulateConnection(
    ...
    // triangle in the middle of 3 hex
  ...
  void TriangulateCornerTerracesCliff(Vector3 begin, HexCell beginCell,
    ...
    TriangulateBoundaryTriangle(begin, color1, left,
      color2, boundary, boundaryColor, types);

    if (leftCell.GetEdgeType(rightCell) == HexEdgeType.Slope) {
      TriangulateBoundaryTriangle(left, color2, right,
        color3, boundary, boundaryColor, types);
    }
    else {
  ...
  Vector3 left, HexCell leftCell, Vector3 right, HexCell rightCell) {
    ...
    TriangulateBoundaryTriangle(right, color3, begin,
      color1, boundary, boundaryColor, types);
  ...
  void TriangulateBoundaryTriangle(Vector3 begin, Color beginColor,
    ...
    for (int i = 2; i < HexMetrics.terraceSteps; i++) {
      ...
      terrain.AddTriangleColor(c2, c2, boundaryColor);
      terrain.AddTriangleTerrainTypes(types);
    }
  ...
  void TriangulateRiverQuad(Vector3 v1, Vector3 v2, Vector3 v3, 
    Vector3 v4, float y1, float y2, float v, bool reversed) 
    {
    v1.y = v2.y = y1;
    v3.y = v4.y = y2;
  ...
  void TriangulateRiverQuad(Vector3 v1, Vector3 v2, Vector3 v3,
    Vector3 v4, float y, float v, bool reversed) {
  ...
  //-------------------ROADS----------------------
    ...
    //road segment at the edges between cells
  ...
  //road across the cell in any direction
  void TriangulateRoad(Vector3 center, Vector3 mL, Vector3 mR, 
    EdgeVertices e, bool hasRoadTrhoughCellEdge) {

    if (hasRoadTrhoughCellEdge) {
  ...
  void TriangulateRoadAdjacentToRiver(
    ...
    bool hasRoadTrhoughEdge = cell.HasRoadThroughEdge(direction);
    ...
    else if (previousHasRiver && nextHasRiver) {   //inside river's curve 
      if (!hasRoadTrhoughEdge) {
        return;
      }
    ...
    TriangulateRoad(roadCenter, mL, mR, e, hasRoadTrhoughEdge);
  ...
  void TriangulateOpenWater(HexDirection direction, HexCell cell,
  HexCell neighbor, Vector3 center) {
    //center hex
    ...
    if (direction <= HexDirection.SE && neighbor != null) {
      ...
      //fill corners between bridges and hexs
      if (direction <= HexDirection.E) {
        HexCell nextNeihbor = cell.GetNeighbor(direction.Next());
        if (nextNeihbor == null || !nextNeihbor.IsUnderWater) {
          return;
        }
        water.AddTriangle(
  ...
  void TriangulateWaterShore(HexDirection direction, HexCell cell,
    HexCell neighbor, Vector3 center) {

    //center hex expansion
